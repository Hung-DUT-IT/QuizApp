package com.example.quizapp.View;

import android.os.Bundle;
import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import androidx.annotation.NonNull;
import androidx.annotation.Nullable;
import androidx.fragment.app.Fragment;
import androidx.navigation.Navigation;
import androidx.recyclerview.widget.LinearLayoutManager;

import com.example.quizapp.R;
import com.example.quizapp.ViewModel.CategoryAdapter;
import com.example.quizapp.databinding.FragmentChooseCategoryBinding;

import java.util.ArrayList;

/**
 * A simple {@link Fragment} subclass.
 * Use the {@link ChooseCategoryFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class ChooseCategoryFragment extends Fragment implements CategoryAdapter.CategoryListener {

    // TODO: Rename parameter arguments, choose names that match
    // the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";
    private FragmentChooseCategoryBinding binding;

    // TODO: Rename and change types of parameters
    private String mParam1;
    private String mParam2;
    Bundle bundle;

    public ChooseCategoryFragment() {
        // Required empty public constructor
    }

    /**
     * Use this factory method to create a new instance of
     * this fragment using the provided parameters.
     *
     * @param param1 Parameter 1.
     * @param param2 Parameter 2.
     * @return A new instance of fragment ChooseCategoryFragment.
     */
    // TODO: Rename and change types and number of parameters
    public static ChooseCategoryFragment newInstance(String param1, String param2) {
        ChooseCategoryFragment fragment = new ChooseCategoryFragment();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, param1);
        args.putString(ARG_PARAM2, param2);
        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
            mParam1 = getArguments().getString(ARG_PARAM1);
            mParam2 = getArguments().getString(ARG_PARAM2);
        }
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        binding= FragmentChooseCategoryBinding.inflate(inflater,container,false);
        return binding.getRoot();

    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        binding.rvCategory.setLayoutManager(new LinearLayoutManager(getActivity().getApplicationContext()));
        ArrayList<String> categoryList = new ArrayList<>();
        categoryList.add("Science");
        categoryList.add("Film & TV");
        categoryList.add("Society & Culture");
        categoryList.add("Food & Drink");
        categoryList.add("Geography");
        categoryList.add("Music");
        categoryList.add("Arts & Literature");
        categoryList.add("History");
        categoryList.add("General Knowledge");
        categoryList.add("Sport & Leisure");
        CategoryAdapter categoryAdapter= new CategoryAdapter(categoryList,this);
        binding.rvCategory.setAdapter(categoryAdapter);
        binding.btnNext.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view1) {
                Navigation.findNavController(view).navigate(R.id.fragmentPagerCreatingQuiz,bundle);
            }
        });
    }

    @Override
    public void sendDataToFragment(String category) {
        bundle = new Bundle();
        bundle.putString("category",category);
        //Navigation.findNavController(view).navigate(R.id.createQuizFragment,bundle);
    }

}
